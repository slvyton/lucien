...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "Unknown Stripe webhook processing error";
    console.error("stripe-webhook processing failed", { type: event.type, event_id: event.id, message });
    return json({ received: true, processing_error: message });
  }
